...
import type { Venue } from "@/lib/coffee-chat/types"
...
export function CoffeeChatInviteModal({ user, sharedInterests = [], children }: CoffeeChatInviteModalProps) {
  ...
  const conversationStarters = selectedPurpose
    ? getConversationStartersForPurpose(selectedPurpose, sharedInterests)
    : []
  ...
  const handleSendInvitation = () => {
    if (!selectedVenue || !selectedPurpose || !selectedDate || !selectedTime || !duration) {
      toast.error("Please fill in all required fields")
      return
    }

    // Here you would typically send the invitation to your backend
    console.log("Sending coffee chat invitation:", {
      toUser: user,
      venue: selectedVenue,
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl border-pink-500/20 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Set up a coffee chat to connect and share ideas</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <Label className="text-base font-medium">What's the purpose of this coffee chat?</Label>
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-pink-500 mt-1">~{purpose.suggestedDuration} min</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          {/* Venue Selection */}
          <div className="space-y-3">
            <Label className="text-base font-medium">Choose a venue</Label>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-start justify-between">
                        <div className="flex items-center gap-2">
                          <span className="text-lg">{getVenueIcon(venue.type)}</span>
                          <div>
                        ...
                        <Badge variant="outline" className={`text-xs ${getAtmosphereColor(venue.atmosphere)}`}>
                          {venue.atmosphere}
                        </Badge>
                      </div>
          ...
          {conversationStarters.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <Label className="text-base font-medium">Conversation Starters</Label>
                <Button
                  variant="ghost"
                  size="sm"
                ...
                <Card className="border-yellow-400/20 bg-gradient-to-r from-yellow-400/5 to-blue-500/5">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs">
                      These will be shared with both of you after the invitation is accepted
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    {conversationStarters.map((starter, index) => (
                      <div key={starter.id} className="text-sm p-2 rounded-lg bg-background/50">
                        <span className="text-yellow-400 font-medium">{index + 1}.</span> {starter.question}
                      </div>
                    ))}
                  </CardContent>
                </Card>
