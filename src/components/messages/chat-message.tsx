    ...
    return (
      <div className="space-y-4">
        ...
        {messages.map((message) => (
          ...
          <div
            key={message.id}
            className={`flex ${message.senderId === sender ? "justify-end" : "justify-start"} gap-2 max-w-[80%] ${
              message.senderId === sender ? "ml-auto" : "mr-auto"
            }`}
          >
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">{message.createdAt ? message.createdAt.toLocaleString() : "Just now"}</p>
            </div>
          </div>
        ))}
      </div>
